   ...
   do
      ...
      ! detector geometry
         ...
         if (strcomp_config(value, c_uniform_square)) then
            ...
            write(*,'(a)') "Value '" // trim(value) // "' for parameter '" //  &
                           c_detector_geometry // "' is unknown. " //          &
      ...
      ! name of levels
         ...
         if (output%nlevels == 0) then
            write(*,'(a)') "In the configuration file, the parameter '" //     &
                           c_level_names // "' should be after '" //           &
                           c_level_number // "'."
            stop
         end if
      ...
      ! offset unit
         ...
         if (strcomp_config(value, c_arcsec)) then
            ...
            write(*,'(a)') "Value '" // trim(value) // "' for detector offset unit is unknown. It can only be '" // c_arcsec // "'"
         end if
      ...
      ! description of detectors
         ...
         if (output%nvertices == 0) then
            write(*,'(a)') "In the configuration file, the parameter '" //     &
                           c_detector_description // "' should be after '" //  &
                           c_detector_geometry // "'."
            stop
         end if
         ...
         allocate(output%vertex(output%ndimensions, output%nvertices,          &
                                output%ndetectors))
         ...
         do idetector = 1, output%ndetectors
            do
               ...
               read(unit,'(a)',iostat=iostat) line
               if (iostat /= 0) then
                  write(*,'(a)') 'Read error, make sure the number of detector description records matches the number of detectors.'
                  stop
               end if
            ...
            read(line,*, iostat=iostat) output%id(:,idetector),                &
                 output%vertex(:,:,idetector), output%flag(idetector)
            if (iostat /= 0) then
               ! the description has no flag
               read(line,*, iostat=iostat) output%id(:,idetector),             &
                    output%vertex(:,:,idetector)
               if (iostat == 0) then

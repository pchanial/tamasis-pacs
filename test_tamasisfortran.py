...
ndetectors = tmmf.pacs_info_ndetectors(filename, True)
ij = tmmf.pacs_info_ij(filename, True, ndetectors)
...
header = tmmf.pacs_map_header_file(filename, True, first, last, fine_sampling_factor, compression_factor)
signal, mask = tmmf.pacs_timeline(filename, True, first, last, ndetectors)
...
tmmf.pacs_pointing_matrix_file(filename, True, npixels_per_sample, first, last, ndetectors, fine_sampling_factor, compression_factor, header, pmatrix)
...
tmmf.pacs_projection_sharp_edges_transpose(pmatrix, signal, map2d, npixels_per_sample)
tmmf.pacs_projection_sharp_edges_direct(pmatrix, map2d, signal, npixels_per_sample)
tmmf.apply_mask(signal, mask)
...
header = tmmf.pacs_map_header_array(time, ra, dec, pa, chop, 'blue', True)
tmmf.pacs_pointing_matrix_array(time, ra, dec, pa, chop, npixels_per_sample, ndetectors, 'blue', True, header, pmatrix)
